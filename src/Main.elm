...
type alias Model =
    ...
    , draggedComponent : Maybe { id : ID, initialPosition : Position }
    , isMouseUp : ID
...
update msg model =
    case msg of
        ...
        OnDragStart id ->
            ...
            in
            ( { model
                | draggedComponent = Just { id = id, initialPosition = component.position }
              }
            , Cmd.none
            )
                            ...
                            List.map
                                (\component ->
                                    if component.id == draggedComponent.id then
                                        { component
                                            | position =
                                                Position draggedComponent.initialPosition.x
                                                    draggedComponent.initialPosition.y
                                        }
...
getOverlay model =
    ...
    if isDragging model then
        let
            component =
                getComponent model.components <| .id <| Maybe.withDefault { id = "", initialPosition = Position 0 0 } model.draggedComponent
